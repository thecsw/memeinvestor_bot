    ...
    while True:
        ...
        for investment in q.limit(10).all():
            ...
            if change > amount:
                ...
                response.edit_wrap(message.modify_invest_return(text, change, update[Investor.balance]))
            elif change == amount:
                logging.info("%s broke even and got back %d" % (investor.name, change))
                response.edit_wrap(message.modify_invest_break_even(text, change, update[Investor.balance]))
            else:
                lost_memes = int( amount - change )
                logging.info("%s lost %d" % (investor.name, lost_memes))
                response.edit_wrap(message.modify_invest_lose(text, lost_memes, update[Investor.balance]))
