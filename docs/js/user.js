...
var getUser = (function() {
	...
	function show(device,pushState=true) {
		const username = document.getElementById(ids.searchBar[device]).value
	...
	function checkUrl() {
		...
		const username = url.searchParams.get("account")
	...
	function init() {
		...
		const searchButton = document.getElementById(ids.searchButton["desktop"])
...
const pageManager = (function() {
	...
	function display(data, firmData) {
		...
		//update name
		const nameEl = document.getElementById(ids.username)
		const firmEl = document.getElementById(ids.firm)
...
const investments = (function() {
	...
	function toggleFilters() {
		const toggleBt = document.getElementById(ids.toggleFilters)
		const tableControls = document.getElementById(ids.tableControls)
		if (filtersOpen) {
			filtersOpen = false
	...
	function applyFilters() {
		if (!loading) {
			const dateFrom = document.getElementById(ids.dateFrom)
			const dateTo = document.getElementById(ids.dateTo)
	...
	function render(data) {
      ...
      `
		for (const inv of data) {
			const invTime = new Date(inv.time*1000)
			const time = invTime.toLocaleTimeString([], {hour: "2-digit", minute:"2-digit", hour12: false})
			const date = invTime.getDate()+"/"+monthNames[invTime.getMonth()]
			html += `
         <tr>
			...
			if (inv.done) {
				const color = inv.success ? "green-text" : "red-text text-lighten-1"
				const sign = inv.success ? "<i class=\"material-icons\">arrow_drop_up</i>" : "<i class=\"material-icons\">arrow_drop_down</i>"
				const profit = sign + formatToUnits(Math.abs(inv.profit - inv.profit * (inv.firm_tax / 100)))
				const finalUpvotes = inv.final_upvotes ? formatToUnits(inv.final_upvotes) : "--"
				const firmTax = inv.firm_tax > 0 ? html += `<br><span title="${inv.firm_tax}% taxed by firm">${inv.firm_tax}%</span>` : ""
				html += `<span class="${color}" title="${commafy(inv.profit)} MemeCoins">${profit} M&cent;</span><br><span title="${commafy(inv.final_upvotes)} upvotes">${finalUpvotes} &uarr;</span>${firmTax}`
			} else {
				const currentTime = new Date()
				//14400000 == 4h
				const millisecondsLeft = 14400000 - ( currentTime.getTime() - invTime.getTime() )
				if (millisecondsLeft >= 0 ) {
					const hoursLeftString = Math.floor(millisecondsLeft / 3600000).toString()
					...
					const timeLeftString = hoursLeftString+":"+minutesLeftString
...
const profitChart = (function() {
	...
	function update() {
		if (desktop) {
			const chartDataSet = ch1.data.datasets[0]
			const data = tempData
