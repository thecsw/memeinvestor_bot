...
const overviewChart = (function() {
	...
	function update(dataSet, index, val) {
		const chartDataSet = ch1.data.datasets[dataSet]
	...
	function init() {
		...
		const displayAxysLabel = desktopRatio
...
const leaderboard = (function() {
	function update(top) {
		const tb = document.getElementById("leaderboards-table")
		let html = ""
		for (let i=0; i<top.length;i++) {
			//broke badge
			const badge = top[i].broke>0? "<span class=\"red bankrupt-badge white-text\">"+top[i].broke+"</span>":""
...
const investmentsCalculator = (function() {
	...
	function calc() {
		const start = parseInt(document.getElementById("investment-start-score").value)
		const end = parseInt(document.getElementById("investment-end-score").value)
...
(function() {
	document.addEventListener("DOMContentLoaded", function() {
		...
		const dataUpdater = new Scheduler(
			function() {
		...
		iterateDays(7, function(index, dateFrom, to) {
			...
			const ufrom = dateFrom.valueOf() / 1000
			const uto = to.valueOf() / 1000
